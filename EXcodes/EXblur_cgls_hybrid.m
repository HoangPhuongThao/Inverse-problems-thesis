...
if strcmp(dispres, 'subplots')
    figure(1), clf
    subplot(2,3,1.5), imagesc(reshape(x, ProbInfo.xSize)), axis off, axis image, colormap(gray)
    title('True solution','interpreter','latex','fontsize',18)
    set(gca,'fontsize',10)
    %
    subplot(2,3,2.5), imagesc(reshape(bn, ProbInfo.bSize)), axis off, axis image, colormap(gray)
...
%     set(gca,'fontsize',10)
    %
    subplot(2,3,4), imagesc(reshape(IterInfo_cgls.BestReg.X, ProbInfo.xSize)), axis off, axis image, colormap(gray)
    title(['Best CGLS sol., $k$ = ' num2str(IterInfo_cgls.BestReg.It)],...
    'interpreter','latex','fontsize',18)
    set(gca,'fontsize',10)
    %
    subplot(2,3,5), imagesc(reshape(X_cgls_dp, ProbInfo.xSize)), axis off, axis image, colormap(gray)
    title(['DP CGLS sol., $k$ = ',num2str(IterInfo_cgls_dp.StopReg.It)],...
    'interpreter','latex','fontsize',18)
    set(gca,'fontsize',10)
    %
    subplot(2,3,6), imagesc(reshape(IterInfo_hybrid.StopReg.X, ProbInfo.xSize)), axis off, axis image, colormap(gray)
