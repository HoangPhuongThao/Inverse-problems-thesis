...
tic;
[X, IterInfo_cgls] = IRcgls_projected(A, bn, options);
time_cgls = toc;
...
options = IRset(options, 'RegParam', 'discrep', 'NoStop', 'on', 'NoiseLevel', NoiseLevel);
tic;
[X_cgls_dp, IterInfo_cgls_dp] = IRcgls_projected(A, bn, options);
time_cgls_dp = toc;
...
if strcmp(dispres, 'subplots')
    ...
    title(['DP CGLS projected sol., $k$ = ',num2str(IterInfo_cgls_dp.StopReg.It)],...
    'interpreter','latex','fontsize',18)
    set(gca,'fontsize',10)
    %
